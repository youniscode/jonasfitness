...
type Variant = "bench" | "press" | "fly" | "pull" | "row" | "squat" | "legpress" | "lunge" | "hinge" | "curl" | "calf" | "core" | "carry";
...
function Figure({ variant }: { variant: Variant }) {
...
export default function ExerciseVisual({ name, imageUrl = "", compact = false }: Props) {
  ...
  return <div className={`exercise-visual-svg${compact ? " compact" : ""}`} role="img" aria-label={`${name} exercise illustration`}><svg viewBox="0 0 220 160" aria-hidden="true"><path className="visual-grid" d="M20 135h180M20 105h180M20 75h180M20 45h180"/><Figure variant={variantFor(name)} /></svg><span>{name}</span></div>;
}
