...
import { optimizeMealBuilder, type MealBuilderStore, type OptimizeRequestMeal } from "../../../lib/nutrition-meal-builder-server.ts";

type OptimizeBody = {
  clientId?: number;
  meals?: OptimizeRequestMeal[];
};
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => ({}))) as OptimizeBody;
  const clientId = Number(body.clientId);

  const store = await loadStore(clientId, ownerId);
  const result = optimizeMealBuilder(ownerId, clientId, body.meals ?? [], store);
