...
// Canonical values stored on the lead (English, coach-facing); labels are
// localized for the prospect. The API accepts any coachingFormat string.
type GoalValue = "Build muscle" | "Build strength" | "Fat loss" | "General fitness" | "Return to training" | "Other";
...
const choices: Record<Lang, { goals: [GoalValue, string][]; experience: [ExperienceValue, string][]; days: number[]; formats: [FormatValue, string][]; contacts: [ContactValue, string][] }> = {
  fr: {
    goals: [["Build muscle","Prendre du muscle"],["Fat loss","Perdre du gras"],["Build strength","Devenir plus fort"],["General fitness","Améliorer ma condition physique"],["Return to training","Reprendre le sport"],["Other","Autre"]],
  ...
  en: {
    goals: [["Build muscle","Build muscle"],["Fat loss","Lose body fat"],["Build strength","Get stronger"],["General fitness","Improve my fitness"],["Return to training","Return to training"],["Other","Something else"]],
  ...
  ar: {
    goals: [["Build muscle","بناء العضلات"],["Fat loss","خسارة الدهون"],["Build strength","زيادة القوة"],["General fitness","تحسين لياقتي"],["Return to training","العودة إلى التدريب"],["Other","شيء آخر"]],
...
export default function CoachingApplication({ lang }: { lang: Lang }) {
  ...
  function show() { setStartedAt(Date.now()); setSuccess(false); setError(""); setStep(0); setGoal(""); setExperience(""); setTrainingDays(3); setFormat(""); setContact("WhatsApp"); setOpen(true); }
  function next() { if (step === 0 && !goal) return; if (step === 1 && !experience) return; if (step === 2 && !trainingDays) return; if (step === 3 && !format) return; setError(""); setStep((current) => Math.min(TOTAL_STEPS - 1, current + 1)); }
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      goal: goal || "General fitness", experience: experience || "", trainingDays, coachingFormat: format || "Online",
  ...
  return <><section className="early-section" id="early-access"><p className="eyebrow"><span/>{t.soon}</p><h2>{t.ready[0]}<br/><em>{t.ready[1]}</em></h2><p>{t.readyText}</p><button className="button button-light" type="button" onClick={show}>{t.open}<span>→</span></button><small>{t.legal}</small></section>
    {open ? <div className="application-backdrop" role="presentation" onMouseDown={() => setOpen(false)}><section className="application-modal" role="dialog" aria-modal="true" aria-labelledby="application-title" onMouseDown={(event) => event.stopPropagation()}><header><div><p>JONAS FITNESS · {t.soon}</p><h2 id="application-title">{success ? t.success : t.title}</h2><span>{success ? t.successText : t.intro}</span></div><button type="button" aria-label={t.close} onClick={() => setOpen(false)}>×</button></header>{success ? <div className="application-success"><i>✓</i><button type="button" className="button" onClick={() => setOpen(false)}>{t.close}<span>→</span></button></div> : <form onSubmit={submit} className="application-wizard"><input className="application-honeypot" name="website" tabIndex={-1} autoComplete="off" aria-hidden="true"/>
      ...
      {step === 0 ? <fieldset className="application-step"><legend>{t.q1}</legend><div className="application-card-grid">{options.goals.map(([value, label]) => <button type="button" key={value} className={`application-card${selected(value, goal)}`} onClick={() => setGoal(value)}>{label}</button>)}</div></fieldset> : null}
