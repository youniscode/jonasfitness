...
export default function CoachingApplication({ lang }: { lang: Lang }) {
  ...
  const [open, setOpen] = useState(false); const [sending, setSending] = useState(false); const [success, setSuccess] = useState(false); const [result, setResult] = useState("created"); const [error, setError] = useState(""); const [startedAt, setStartedAt] = useState(() => Date.now());
  ...
  function show() { setStartedAt(Date.now()); setSuccess(false); setResult("created"); setError(""); setStep(0); setGoals({ primary: "", secondary: [] }); setExperience(""); setTrainingDays(3); setFormat(""); setContact("WhatsApp"); setOpen(true); }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { const response = await fetch("/api/apply", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error ?? t.error); setResult(String(data.result ?? "created")); setSuccess(true); }
  ...
  return <><section className="early-section" id="early-access"><p className="eyebrow"><span/>{t.soon}</p><h2>{t.ready[0]}<br/><em>{t.ready[1]}</em></h2><p>{t.readyText}</p><button className="button button-light" type="button" onClick={show}>{t.open}<span>→</span></button><small>{t.legal}</small></section>      {open ? <div className="application-backdrop" role="presentation" onMouseDown={() => setOpen(false)}><section className="application-modal" role="dialog" aria-modal="true" aria-labelledby="application-title" onMouseDown={(event) => event.stopPropagation()}><header><div><p>JONAS PROGRESS · {t.soon}</p><h2 id="application-title">{success ? t.success : t.title}</h2><span>{success ? (result === "created" || result === "reapplication_created" || result === "duplicate_retry" ? t.successText : t.successReceived) : t.intro}</span></div><button type="button" aria-label={t.close} onClick={() => setOpen(false)}>×</button></header>{success ? <div className="application-success"><i>✓</i><button type="button" className="button" onClick={() => setOpen(false)}>{t.close}<span>→</span></button></div> : <form onSubmit={submit} className="application-wizard"><input className="application-honeypot" name="website" tabIndex={-1} autoComplete="off" aria-hidden="true"/>
