...
const L: Record<Lang, Record<string, string>> = {
  fr: {
    ...
    // Nutrition Foundations V1 / Phase 2A — demographics + safety flags
    "male": "Homme", "female": "Femme", "prefer_not_to_say": "Je préfère ne pas le dire",
    "minor": "Moins de 18 ans", "pregnant": "Enceinte", "eating_disorder_history": "Antécédents de troubles alimentaires", "diabetes": "Diabète", "kidney_disease": "Maladie rénale", "severe_allergy": "Allergie sévère", "therapeutic_diet": "Régime thérapeutique",
  },
  ar: {
    ...
    // Nutrition Foundations V1 / Phase 2A — demographics + safety flags
    "male": "ذكر", "female": "أنثى", "prefer_not_to_say": "أفضّل عدم الإجابة",
    "minor": "أقل من 18 عامًا", "pregnant": "حامل", "eating_disorder_history": "تاريخ من اضطراب الأكل", "diabetes": "مرض السكري", "kidney_disease": "مرض الكلى", "severe_allergy": "حساسية شديدة", "therapeutic_diet": "نظام غذائي علاجي",
  },
  en: NUTRITION_EN_LABELS,
};
...
function labelFor(lang: Lang, key: string): string {
  ...
  // Training-supervision options are canonical stored tokens ("alone" | "coach"
  // | "mixed") — only the DISPLAYED label is localized. The stored profile value
  // stays byte-identical so persistence and solo-beginner logic never change.
  if (key === "alone" || key === "coach" || key === "mixed") {
    return supervisionLabelFor(lang, key) || key;
  }
...
type StepKey = "goals" | "timeline" | "experience" | "confidence" | "schedule" | "venue" | "exercisePreferences" | "preferences" | "limitations" | "lifestyle" | "recovery" | "motivation" | "coaching" | "demographics" | "nutrition" | "nutritionSafety" | "measurements" | "openNote";
...
const STEPS: { key: StepKey; title: string; subtitle?: string }[] = [
  ...
  { key: "demographics", title: "demographicsTitle", subtitle: "demographicsSubtitle" },
  { key: "nutrition", title: "nutritionTitle", subtitle: "nutritionSubtitle" },
  { key: "nutritionSafety", title: "nutritionSafetyTitle", subtitle: "nutritionSafetySubtitle" },
...
const stepDone: Record<StepKey, (profile: OnboardingProfile) => boolean> = {
  ...
  // Optional but useful foundations: answered once age or sex is given.
  demographics: (p) => p.demographics.ageYears !== null || Boolean(p.demographics.sex),
  nutrition: (p) => Boolean(p.nutrition.tracking),
  // Optional safety review: considered answered once the client actively
  // confirmed a flag or added a note — an untouched step reads as "not provided".
  nutritionSafety: (p) => p.nutritionSafety.flags.length > 0 || Boolean(p.nutritionSafety.note),
  ...
  switch (step) {
    case "goals":
      return <div className="ob-step-body">
        ...
        <h3>{t.qTargetWeight}<em>{t.optional}</em></h3>
        <label className="ob-single-input"><input type="number" min={25} max={400} step="0.1" value={profile.goals.targetWeightKg ?? ""} placeholder="kg" onChange={(event) => update((draft) => { draft.goals.targetWeightKg = event.target.value ? Number(event.target.value) : null; })} /></label>
    ...
    case "confidence":
      return <div className="ob-step-body">
        <h3>{t.qTrainingSupervision}</h3>
        <SingleSelect options={TRAINING_SUPERVISIONS as readonly string[]} value={profile.trainingSupervision} onChange={(value) => update((draft) => { draft.trainingSupervision = value as typeof draft.trainingSupervision; })} />
        <h3>{t.qConfidence}</h3>
    ...
    case "demographics":
      return <div className="ob-step-body">
        <h3>{t.qAge}<em>{t.optional}</em></h3>
        <label className="ob-single-input"><input type="number" min={AGE_MIN} max={AGE_MAX} value={profile.demographics.ageYears ?? ""} placeholder={t.ageYearsPlaceholder} onChange={(event) => update((draft) => { draft.demographics.ageYears = event.target.value ? Number(event.target.value) : null; })} /></label>
        <h3>{t.qSex}<em>{t.optional}</em></h3>
        <SingleSelect options={SEX_VALUES as readonly string[]} value={profile.demographics.sex} onChange={(value) => update((draft) => { draft.demographics.sex = value; })} />
        <small className="ob-safety">{t.sexNote}</small>
      </div>;
    case "nutrition":
      return <div className="ob-step-body">
        <h3>{t.qTracking}</h3>
        ...
        <h3>{t.qAllergies}<em>{t.optional}</em></h3>
        <label className="ob-food-list"><textarea value={profile.nutrition.allergies.join(", ")} placeholder={t.foodListPlaceholder} onChange={(event) => update((draft) => { draft.nutrition.allergies = event.target.value.split(/[,\n;]/).map((item) => item.trim()).filter(Boolean); })} /></label>
        <h3>{t.qIntolerances}<em>{t.optional}</em></h3>
        <label className="ob-food-list"><textarea value={profile.nutrition.intolerances.join(", ")} placeholder={t.foodListPlaceholder} onChange={(event) => update((draft) => { draft.nutrition.intolerances = event.target.value.split(/[,\n;]/).map((item) => item.trim()).filter(Boolean); })} /></label>
        <h3>{t.qDisliked}<em>{t.optional}</em></h3>
        <label className="ob-food-list"><textarea value={profile.nutrition.dislikedFoods.join(", ")} placeholder={t.foodListPlaceholder} onChange={(event) => update((draft) => { draft.nutrition.dislikedFoods = event.target.value.split(/[,\n;]/).map((item) => item.trim()).filter(Boolean); })} /></label>
        <h3>{t.qMealsPerDay}<em>{t.optional}</em></h3>
        <label className="ob-single-input"><input type="number" min={MEALS_PER_DAY_MIN} max={MEALS_PER_DAY_MAX} value={profile.nutrition.mealsPerDay ?? ""} onChange={(event) => update((draft) => { draft.nutrition.mealsPerDay = event.target.value ? Number(event.target.value) : null; })} /></label>
        <Note label={t.noteNutrition} value={profile.nutrition.note} onChange={(value) => update((draft) => { draft.nutrition.note = value; })} />
      </div>;
    case "nutritionSafety":
      return <div className="ob-step-body">
        <h3>{t.qSafetyFlags}</h3>
        <MultiSelect options={NUTRITION_SAFETY_FLAGS as readonly string[]} values={profile.nutritionSafety.flags} onChange={(next) => update((draft) => { draft.nutritionSafety.flags = next; })} />
        <Note label={t.noteNutritionSafety} value={profile.nutritionSafety.note} onChange={(value) => update((draft) => { draft.nutritionSafety.note = value; })} />
        <small className="ob-safety">{t.nutritionSafetyNote}</small>
      </div>;
    case "measurements":
      return <div className="ob-step-body">
