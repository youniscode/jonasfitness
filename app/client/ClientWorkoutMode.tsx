...
export default function ClientWorkoutMode({ lang, preview, clientId }: { lang: Lang; preview: boolean; clientId: number }) {
  ...
  function updateWorkout(change: (current: Workout) => Workout) { setWorkout((current) => current ? change(current) : current); }
  ...
  async function finishWorkout() {
    if (!workout || preview) return;
    setSaving(true);
    const response = await fetch(`/api/client-workouts/${workout.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ status: "completed", exercises: workout.exercises, notes: workout.notes }) });
    const payload = await response.json().catch(() => ({}));
    setSaving(false);
  ...
  const previous = useMemo(() => current && data ? data.history.flatMap((entry) => entry.exercises).find((exercise) => normalise(exercise.name) === normalise(current.name)) ?? null : null, [current, data]);
  const workoutStats = workout ? statsFor(workout.exercises) : { exercises: 0, completedSets: 0, totalVolume: 0 };
  const recordCount = useMemo(() => workout ? workout.exercises.filter((exercise) => {
    const currentMax = Math.max(0, ...exercise.sets.filter((set) => set.status === "completed").map((set) => set.weight ?? 0));
    const pastMax = Math.max(0, ...(data?.history.flatMap((entry) => entry.exercises).filter((past) => normalise(past.name) === normalise(exercise.name)).flatMap((past) => past.sets).filter((set) => set.status === "completed").map((set) => set.weight ?? 0) ?? []));
    return currentMax > pastMax && currentMax > 0;
  }).length : 0, [data?.history, workout]);
