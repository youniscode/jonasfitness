...
import ClientExerciseFeedback from "./ClientExerciseFeedback";
...
type WorkoutExercise = { id: string; programmeExerciseId: string; libraryId: string; name: string; nameFr?: string; nameAr?: string; target: string; focus: string; instructions: string; imageUrl: string; videoUrl: string; restSeconds: number; note: string; status: "pending" | "completed" | "skipped"; sets: WorkoutSet[] };
...
export default function ClientWorkoutMode({ lang, preview, clientId }: { lang: Lang; preview: boolean; clientId: number }) {
  ...
  if (mode === "live" && workout && !current) return <section className="client-workout-overlay"><header><strong>JONAS PROGRESS</strong><button type="button" onClick={() => setMode("choose")}>×</button></header><main className="client-workout-recovery"><h1>{t.recovery}</h1><button type="button" className="portal-button" onClick={() => { localStorage.removeItem(`jonas-client-workout-${workout.id}`); void load(); }}>{t.reload}<span>→</span></button></main></section>;

  if (mode === "live" && workout && current) return <section className="client-workout-overlay"><header><button type="button" onClick={() => setMode("choose")}>← {t.back}</button><strong>JONAS PROGRESS</strong><span>{saving ? t.saving : message || t.autosave}</span></header><main className="client-workout-live"><div className="client-workout-live-head"><div><p>{t.kicker}</p><h1>{workout.title}</h1><span>{t.exercise} {exerciseIndex + 1} {t.of} {workout.exercises.length}</span></div><button type="button" className="portal-button" disabled={saving} onClick={() => void finishWorkout()}>{t.finish}<span>✓</span></button></div><nav aria-label={t.exercise}>{workout.exercises.map((exercise, index) => <button type="button" key={exercise.id} className={index === exerciseIndex ? "active" : exercise.status === "completed" ? "complete" : ""} onClick={() => setExerciseIndex(index)}>{index + 1}{exercise.status === "completed" ? " ✓" : ""}</button>)}</nav><section className="client-exercise-stage"><div className="client-exercise-title"><p>{current.focus}</p>{current.imageUrl && <div className="client-exercise-image" role="img" aria-label={exerciseDisplayName(current, lang)} style={{ backgroundImage: `url(${current.imageUrl})` }} />}<h2>{exerciseDisplayName(current, lang)}</h2><span>{current.target}</span><small className="client-rir-help">{rirHelp[lang]}</small>{current.instructions && <small className="client-exercise-instructions">{current.instructions}</small>}{current.videoUrl && <a className="client-exercise-demo" href={current.videoUrl} target="_blank" rel="noreferrer">Demo ↗</a>}</div><aside><small>{t.lastTime}</small>{previous ? previous.sets.filter(isCompletedWorkoutSet).map((set) => <span key={set.id}>{set.weight ?? "-"} kg × {set.reps ?? "-"}</span>) : <p>{t.noPrevious}</p>}</aside><div className="client-set-table"><div className="client-set-head"><span>#</span><span>{t.weight}</span><span>{t.reps}</span><span>RIR</span><span>{t.previous}</span><span /></div>{current.sets.map((set, index) => <div className={set.status === "completed" ? "client-set-row complete" : "client-set-row"} key={set.id}><strong>{index + 1}</strong><input aria-label={t.weight} inputMode="decimal" type="number" min="0" step="0.5" placeholder="kg" value={set.weight ?? ""} onChange={(event) => updateSet(index, { weight: event.target.value === "" ? null : Number(event.target.value) })}/><input aria-label={t.reps} inputMode="numeric" type="number" min="0" placeholder={set.target || t.reps} value={set.reps ?? ""} onChange={(event) => updateSet(index, { reps: event.target.value === "" ? null : Number(event.target.value) })}/><input aria-label="RIR" inputMode="numeric" type="number" min="0" max="6" placeholder="2" value={set.rir} onChange={(event) => updateSet(index, { rir: event.target.value })}/><span>{previous?.sets[index] && isCompletedWorkoutSet(previous.sets[index]) ? `${previous.sets[index].weight ?? "-"} × ${previous.sets[index].reps ?? "-"}` : "-"}</span><button type="button" onClick={() => toggleSet(index)}>{set.status === "completed" ? "✓" : t.done}</button></div>)}</div><label>{t.note}<textarea value={workout.notes} placeholder={t.noteHint} onChange={(event) => updateWorkout((currentWorkout) => ({ ...currentWorkout, notes: event.target.value }))}/></label></section><section className="client-rest-timer" aria-live="polite"><small>{t.rest}</small><strong>{String(Math.floor(restSeconds / 60)).padStart(2, "0")}:{String(restSeconds % 60).padStart(2, "0")}</strong><button type="button" onClick={() => setRestRunning((running) => !running)} disabled={restSeconds === 0}>{restRunning ? t.pause : t.continue}</button><button type="button" onClick={() => { setRestSeconds(0); setRestRunning(false); }}>{t.reset}</button></section><footer>{exerciseIndex > 0 ? <button type="button" className="client-workout-footer-prev" onClick={() => setExerciseIndex((index) => index - 1)}>← {t.previousExercise}</button> : <span className="client-workout-footer-spacer" aria-hidden="true" />}<span className="client-workout-footer-status">{saving ? t.saving : message || t.autosave}</span><button type="button" className="client-workout-footer-next" disabled={exerciseIndex === workout.exercises.length - 1} onClick={() => setExerciseIndex((index) => index + 1)}>{t.nextExercise} →</button></footer></main></section>;

  return <section className="client-workout-overlay"><header><strong>JONAS PROGRESS</strong></header><main className="client-workout-summary"><p>{t.kicker}</p><h1>{t.complete}</h1><h2>{workout?.title}</h2><span>{t.shared}</span><div><article><small>{t.completedSets}</small><strong>{workoutStats.completedSets}</strong></article><article><small>{t.totalVolume}</small><strong>{workoutStats.totalVolume.toLocaleString(locale[lang])} kg</strong></article><article><small>{t.records}</small><strong>{recordCount}</strong></article></div>{workout && <ClientExerciseFeedback lang={lang} preview={preview} workout={{ id: workout.id, exercises: workout.exercises }} />}<button type="button" className="portal-button" onClick={() => { workoutRef.current = null; setWorkout(null); void load(); }}>{t.back}<span>→</span></button></main></section>;
}
